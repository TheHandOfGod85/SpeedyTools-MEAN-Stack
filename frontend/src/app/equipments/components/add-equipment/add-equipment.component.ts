import { getCurrentEquipment } from './../../state/index'
    ...
    ngOnInit(): void {
        ...
        this.route.params.subscribe((params: Params) => {
            ...
            if (this.equipmentId) {
                ...
                this.store.dispatch(
                    EquipmentPageActions.loadEquipment({ id: this.equipmentId })
                )
                this.equipment$ = this.store
                    .select(getCurrentEquipment)
                    .pipe(
                        tap((currentEquipment) =>
                            this.displayEquipment(currentEquipment)
                        )
                    )
            }
        })
        this.equipment$ = this.store.select(getCurrentEquipment).pipe(
            tap((currentEquipment) => {
                this.displayEquipment(currentEquipment)
            })
        )
